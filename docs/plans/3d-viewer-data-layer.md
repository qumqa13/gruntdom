...
| F1-T7 | `apps/workers/jobs/generate-provenance-screenshots.ts` — Puppeteer headless. Per layer (kataster, MPZP, ortofoto, NMT, klasy gleby, sieci uzbrojenia): render Geoportal page focused on plot bbox + 50 % padding with single layer enabled. Output PNG to R2 path `provenance/{slug}/{layer}-{fetchedAt}.png`. | worker | executor |
| F1-T8 | `apps/workers/jobs/generate-3d-preview-thumbnail.ts` — for opt-in lazy-load placeholder. Geoportal ortofoto crop centered on centroid + bbox padding + SVG-rendered polygon overlay. Output to R2 path `previews/{slug}-3d-preview.jpg`, cache 30 days. | worker | executor |
...
| F2-T8 | Custom controls overlay (HTML, not Cesium): "Z lotu ptaka / Perspektywa" toggle, "Reset widok", "Obróć ±30°", North arrow synced to heading. Atelier styling. | overlay component |
